      ...
      <div className="min-h-screen bg-[#0a0a0a] text-white font-sans">
        ...
        <aside className={`hidden md:block fixed left-0 top-0 h-full bg-[#111111] border-r border-[#222222] transition-all duration-300 z-40 ${sidebarOpen ? 'w-56' : 'w-14'}`}>
          ...
          <nav className="p-2 space-y-1">
            {navItems.map((item) => (
              <a key={item.id} href={item.href} className={`flex items-center px-3 py-2.5 rounded-lg text-sm transition-all ${item.id === 'settings' ? 'text-white bg-[#1a1a1a]' : 'text-[#888888] hover:text-white hover:bg-[#1a1a1a]'}`}>
                <span className="w-5">{item.id === 'settings' ? '●' : '○'}</span>
        ...
        <main className={`md:transition-all md:duration-300 relative z-10 ${sidebarOpen ? 'md:ml-56' : 'md:ml-14'}`}>
          <div className="pt-16 md:pt-0 max-w-5xl mx-auto px-4 md:px-8 py-8">
            <header className="mb-8">
              <h1 className="text-2xl md:text-3xl font-semibold text-white mb-1">Settings</h1>
              <p className="text-[#666666]">App configuration</p>
            </header>

            <section className="mb-8">
              <div className="p-6 bg-[#111111] border border-[#222222] rounded-xl">
                <p className="text-[#555555]">Settings coming soon</p>
              </div>
            </section>
